...
export default function IngredientsClient({ token }: { token: string | null }) {
  ...
  const lowStock = ingredients.filter(i => i.stock_qty <= i.alert_qty && i.alert_qty > 0)

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">{t('ingredients.title')}</h2>
          <p className="text-sm text-slate-400 mt-1">{t('ingredients.subtitle')}</p>
        </div>
      ...
      {lowStock.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 flex items-center gap-2">
          ...
          <span className="text-amber-700 text-sm font-medium">
            {lowStock.length} {t('ingredients.low_stock_warning')}&nbsp;
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-left px-6 py-3 text-slate-500 font-medium">{t('ingredients.unit')}</th>
                <th className="text-left px-6 py-3 text-slate-500 font-medium">{t('ingredients.stock')}</th>
                <th className="text-left px-6 py-3 text-slate-500 font-medium">{t('ingredients.alert_at')}</th>
                <th className="text-left px-6 py-3 text-slate-500 font-medium">{t('ingredients.cost_unit')}</th>
              ...
              {ingredients.map(i => (
                <tr key={i.id} className="border-b border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3">
                    <button onClick={() => openEdit(i)} className="text-indigo-600 hover:text-indigo-800 font-medium mr-4">{t('common.edit')}</button>
                    <button onClick={() => del(i.id, i.name)} className="text-red-500 hover:text-red-700 font-medium">{t('common.delete')}</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {ingredients.length === 0 && <div className="px-6 py-12 text-center text-slate-400">{t('ingredients.no_ingredients')}</div>}
        </div>
      )}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <h3 className="text-lg font-bold text-slate-800 mb-4">{form.id ? t('ingredients.edit') : t('ingredients.new')}</h3>
            <div className="space-y-4">
              {[
                { label: t('common.name'), key: 'name', type: 'text', placeholder: 'e.g. Whole Milk' },
                { label: t('ingredients.stock_qty'), key: 'stock_qty', type: 'number', placeholder: '1000' },
                { label: t('ingredients.alert_qty'), key: 'alert_qty', type: 'number', placeholder: '200' },
                { label: t('ingredients.cost_per_unit'), key: 'cost_per_unit', type: 'number', placeholder: '5' },
              ].map(f => (
                <div key={f.key}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">{t('ingredients.unit')}</label>
